...
charizard <- c("base" = "#207394", "tidytof" = "#cd5241")
```
...
ddpr_data_info %>% 
  write_rds(
    file = here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "ddpr_data_info.rds")
  )
```
...
io_benchmarking_3 %>% 
  write_rds(
    file = here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "io_benchmarking_speed.rds")
  )
```
...
io_speed_plot %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "io_benchmarking_speed.pdf"), 
    device = "pdf", 
    width = 5, 
    height = 3
  )
...
io_benchmarking_3_csv %>% 
  write_rds(
    file = here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "io_benchmarking_speed_csv.rds")
  )
```
...
io_benchmarking_3_csv <- 
  read_rds(file = here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "io_benchmarking_speed_csv.rds"))
...
io_speed_plot_csv %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "io_benchmarking_speed_csv.pdf"), 
    device = "pdf", 
    width = 5, 
    height = 3
  )
...
ddpr_large_tibble <- 
  tof_read_data(ddpr_datasets_mini$file_paths[[nrow(ddpr_datasets)]]) %>% 
  group_by(file_name) %>% 
  nest() %>% 
  ungroup()

ddpr_large_flowset <- 
  flowCore::read.flowSet(files = ddpr_datasets_mini$file_paths[[nrow(ddpr_datasets)]])
...
ddpr_data_large <- 
  ...
  mutate(
    ...
    flowSets = 
      map(
        .x = 1:nrow(ddpr_datasets), 
        .f = ~ddpr_large_flowset[1:.x]
      )
  )

ddpr_data_large$tibbles[[1]]
...
downsample_tidytof(data_frame = ddpr_data_mini$tibbles[[2]])
```
...
```{r, eval = FALSE}
downsample_benchmarking_3 <- 
  ddpr_data_mini %>% 
  #filter(num_files <= 10) %>% 
  mutate(downsample_benchmarks = map(.x = tibbles, .f = benchmark_downsample)) %>% 
...
downsample_benchmarking_3 %>% 
  write_rds(
    file = here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "downsample_benchmarking_speed.rds")
  )
...
downsample_speed_plot %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "downsample_benchmarking_speed.pdf"), 
    device = "pdf", 
    width = 5, 
    height = 3
  )
...
```{r, eval = FALSE}
preprocess_benchmarking_3 <- 
  ddpr_data_mini %>% 
  #filter(num_files <= 10) %>% 
  mutate(preprocess_benchmarks = map(.x = tibbles, .f = benchmark_preprocess)) %>% 
  select(-tibbles, -flowSets)
```
...
preprocess_benchmarking_3 %>% 
  write_rds(
    file = here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "preprocess_benchmarking_speed.rds")
  )
```
...
preprocess_speed_plot %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "preprocess_benchmarking_speed.pdf"), 
    device = "pdf", 
    width = 5, 
    height = 3
  )
...
dr_tibbles <- 
  tibble(
    sample_cells = (seq(1000, 10^5, length.out = 10)), 
    data_frames = 
      map(
        .x = sample_cells, 
...
```{r, eval = FALSE}
time <- 
  benchmark_tsne(data_frame = dr_tibbles$data_frames[[2]] )
time 
...
tsne_benchmark_3 <- 
  ...
  mutate(
    ...
    tsne_benchmarks = map(.x = data_frames, .f = benchmark_tsne)
  ) %>% 
  select(-data_frames) %>% 
...
pca_benchmark_3 <- 
  dr_tibbles %>%
  mutate(pca_benchmarks = map(.x = data_frames, .f = benchmark_pca)) %>% 
  select(-data_frames) %>% 
  rename(num_cells = sample_cells)
...
tsne_benchmark_3 %>% 
  write_rds(
    file = here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "tsne_benchmarking_speed.rds")
  )
```
...
tsne_speed_plot %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "tsne_benchmarking_speed.pdf"), 
    device = "pdf", 
    width = 5, 
    height = 3
  )
...
pca_benchmark_3 %>% 
  write_rds(
    file = here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "pca_benchmarking_speed.rds")
  )
```
...
pca_speed_plot %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "pca_benchmarking_speed.pdf"), 
    device = "pdf", 
    width = 5, 
    height = 3
  )
...
cluster_benchmarking_3 %>% 
  write_rds(
    file = here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "flowsom_benchmarking_speed.rds")
  )
```
...
flowsom_speed_plot %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "flowsom_benchmarking_speed.pdf"), 
    device = "pdf", 
    width = 5, 
    height = 3
  )
...
extract_benchmarking_3 %>% 
  write_rds(
    file = here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "extract_benchmarking_speed.rds")
  )
```
...
extract_speed_plot %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "extract_benchmarking_speed.pdf"), 
    device = "pdf", 
    width = 5, 
    height = 3
  )
...
lines_plot_1 %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "lines_plot_1.pdf"), 
    device = "pdf", 
    width = 5, 
    height = 3.5
  )
...
lines_plot_2 %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "lines_plot_2.pdf"), 
    device = "pdf", 
    width = 6, 
    height = 4
  )
...
assignment_plot_1 %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "assignment_plot_1.pdf"), 
    device = "pdf", 
    width = 4.5, 
    height = 3.5
  )
...
assignment_plot_2 %>% 
  ggsave(
    filename = 
      here::here("notes", "manuscript", "nature_methods", "supplement", "benchmarking", "assignment_plot_2.pdf"), 
    device = "pdf", 
    width = 7, 
    height = 5
  )
