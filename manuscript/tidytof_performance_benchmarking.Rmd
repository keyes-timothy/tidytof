...
high_level_apis <- c("tidytof", "cytofkit", "spectre", "immunocluster")
low_level_apis <- c("tidytof", "flowcore", "base")
...
plot_benchmarks <- 
  function(
    ...
    if (api == "all") { 
      benchmark_data <- 
        benchmark_data %>% 
        select(num_files, num_cells, last_col()) %>% 
        unnest(last_col()) 
    } else if (api == "high") { 
         benchmark_data <- 
           benchmark_data %>% 
...
plot_benchmarks_split <- function(
  ...
  full_plot <- 
    benchmark_data %>% 
    plot_benchmarks(subtitle = subtitle, api = "all")
  
  high_plot <- 
    benchmark_data %>% 
    plot_benchmarks(subtitle = subtitle, api = "high") 
  
  low_plot <- 
    benchmark_data %>% 
    plot_benchmarks(subtitle = subtitle, api = "low")
...
io_speed_plots <- 
  io_benchmarking %>% 
  plot_benchmarks_split(subtitle = "FCS reader")
...
downsample_speed_plots <- 
  downsample_benchmarking %>% 
  plot_benchmarks_split(subtitle = "Downsampling")
...
preprocess_benchmarking %>%
  select(num_cells, preprocess_benchmarks) %>% 
  unnest() %>% 
  group_by(num_cells, engine) %>% 
...
preprocess_speed_plots <- 
  preprocess_benchmarking %>% 
  plot_benchmarks_split(subtitle = "Preprocessing")
...
tsne_speed_plots <- 
  tsne_benchmark %>% 
  plot_benchmarks_split(subtitle = "tSNE")

tsne_speed_plot <- tsne_speed_plots$
