...
To benchmark {tidytof}'s performance against existing tools for high-dimensional cytometry data analysis, we compared {tidytof} functions to their equivalent workflows using base R @baseR or the flowCore package @flowcore where equivalent workflows existed. Workflows were compared both on the basis of their computational speed and on the basis of their coding burden (i.e. the amount of code needed for each workflow). Specifically, comparisons included one metric for computational speed (total elapsed time for each workflow) and two metrics for coding burden associated with human error when conducting computational analyses (the number of lines of code and the number of variable assignments needed to write the workflow).

All benchmarking was performed on a Linux machine running Ubuntu 20.04 with an AMD Threadripper Pro 3995WX processor (64 cores; 2.70 GHz; 256 MB cache) and 256 GB of RAM.


### Benchmarking datasets

The data used for benchmarking were drawn from a publicly available CyTOF dataset published in 2018 @ddpr. Specifically, 20 files acquired from 2 healthy patients were chosen from the full dataset, totaling `r total_num_cells` cells. The following files were used: 
...
Using these datasets, the `microbenchmark` package @microbenchmark was used to time each workflow on each dataset 10 independent times. The median and interquartile range of each workflow's runtime could then be computed for {tidytof}, base R, and flowCore. For dimensionality reduction and clustering analyses, only cluster of differentiation ("CD") markers were used for analysis across all workflows. For downsampling, 200 cells were subsampled from each input FCS or CSV file. Otherwise, {tidytof}'s default behavior was used across all functions used for speed benchmarking.

### Coding burden benchmarking

For coding burden benchmarking, the R scripts containing each workflow were themselves subjected to text analysis. To control for differences in code style and practices between workflows, the styler package @styler was used to standardize all benchmarking code to adhere to the [tidyverse R code style guide](https://style.tidyverse.org/). The functions used to count the number of lines of code in each workflow are provided here:
...
Across a wide variety of functions including FCS and CSV file reading, single-cell data transformation (preprocessing), downsamping, tSNE, FlowSOM clustering, and sample-level feature extraction, 
...
Despite its competitive speed performance relative to equivalent base R and {flowCore} workflows, {tidytof} reduces the coding burden of performing CyTOF data analyses significantly compared to alternative frameworks. Across workflows, {tidytof} reduces the lines of code needed to perform an analysis between 70% and 95% compared to base R and between 56% and 96% compared to {flowCore} (**Supplementary Figure 2**). Similarly, {tidytof} reduces the number of variable assignments needed to perform an analysis between 75% to 97% compared to base R and between 75% and 95% compared to {flowCore} (**Supplementary Figure 3**). Because code bases with both more lines of code and more intermediate variables are more prone to human (e.g copy-and-paste) error and reduced readability, these results support {tidytof}'s marked ability to simplify CyTOF data analysis code and increase reprodubility among data analysis pipelines.

```{r}
make_style_plot <- function(raw, base, flowcore) { 
...
**Supplementary Figure 4 - {tidytof}'s tidy data structures require similar system memory to {flowCore} flowSet objects.** Despite the increased simplicity of {tidytof}'s tidy data structures (`tof_tbl` objects),  
