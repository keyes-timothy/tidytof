...
description: > 
  Read this vignette to learn how to downsample a CyTOF dataset to a smaller 
  number of cells using {tidytof}.
vignette: >
  %\VignetteIndexEntry{Dowsampling}
...
Often, CyTOF experiments collect tens or hundreds or millions of cells in total, and it can be useful to downsample to a smaller, more computationally tractable number of cells - either for a final analysis or while developing code. 
...
Thus, we can see that the density after downsampling is approximately uniform across the range of `cd34`/`cd38` values in `phenograph_data`.
